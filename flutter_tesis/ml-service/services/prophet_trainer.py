...
class ProphetTrainer:
    ...
    def _prepare_prophet_data(self, events_history: list) -> pd.DataFrame:
        ...
        for event in events_history:
            try:
                ...
                # Convertir a timezone-naive (Prophet funciona mejor sin timezone)
    ...
    def _calculate_weeks_of_data(self, df: pd.DataFrame) -> float:
        ...
        try:
            # Normalizar fechas a timezone-naive para comparación
            first_date = df['ds'].min()
            last_date = df['ds'].max()
            
            # Convertir a pandas datetime si no lo son
            ...
            # Fallback: asumir 2 semanas si hay suficientes eventos
            return len(df) / 10.0
