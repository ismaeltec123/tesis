...
class GoogleCalendarService:
    ...
    def _convert_from_google_format(self, google_event: Dict[str, Any], preserve_type: str = None, preserve_imported: bool = None) -> Dict[str, Any]:
        ...
        # Zona horaria local (Lima UTC-5)
        local_tz = pytz.timezone('America/Lima')
        
        if start_time:
            start_time = datetime.fromisoformat(start_time.replace('Z', '+00:00'))
            # Convertir a hora local si tiene zona horaria
        ...
        # Usar el tipo preservado si se proporciona, sino usar 'recreativo' por defecto
        event_type = preserve_type if preserve_type else 'recreativo'
        
        # Usar el estado de importado preservado, o True por defecto (viene de Google)
        is_imported = preserve_imported if preserve_imported is not None else True
        
        # Determinar el estado inicial del evento
        event_status = 'pendiente'  # Por defecto, eventos nuevos están pendientes
        if start_time:
            # Si el evento ya pasó, marcarlo como no_realizado
            now = datetime.now(timezone.utc)
            # Comparar ambos como timezone-aware
            start_time_aware = start_time if start_time.tzinfo else start_time.replace(tzinfo=timezone.utc)
            if start_time_aware < now:
                event_status = 'no_realizado'
        
        return {
            'title': google_event.get('summary', 'Sin título'),
            'description': google_event.get('description', ''),
            # Enviar en formato ISO sin zona horaria (hora local)
            'date': start_time.replace(tzinfo=None).isoformat() if start_time else None,
            'end_time': end_time.replace(tzinfo=None).isoformat() if end_time else None,
            'type': event_type,
            'status': event_status,
